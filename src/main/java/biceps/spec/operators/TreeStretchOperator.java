...
public class TreeStretchOperator extends EpochFlexOperator {
    final public Input<RealScalarParam<PositiveReal>> meanRateInput = new Input<>("meanRate", "mean clock rate -- inversely scaled if specified");
	
	// TODO: make up/down version that takes clock rates in account
	
	private double [] oldLengths;
	private double logHR;
	private RealScalarParam<PositiveReal> meanRate = null;
	...
	public double proposal() {
		...
		if (meanRate != null) {
			...
			double newRate = meanRate.get() * oldHeight/newHeight;
			
			if (newRate < meanRate.getLower() || newRate > meanRate.getUpper()) {
				return Double.NEGATIVE_INFINITY;
			}
			meanRate.set(newRate);
